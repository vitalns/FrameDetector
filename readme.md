...
1. Установка  
    1.1. Шаги    
        1.1.1. Скачайте *`program.zip`*  
        1.1.2. Распакуйте:  
            ```bash
            unzip program.zip -d /opt/
            ```
    1.2. Проверка
2. Настройка
    2.1. Конфиг
        2.1.1. Откройте _`/etc/app/config.conf`_  
        2.1.2. Добавьте:  
            ```ini
            [main]
            port = 8080
...
для генерации потока кадров;        
        5.5.2 **generate_video** для генерации видеофайла AVI;                 5.5.3 **generate_random_frames** для генерации случайных
кадров;         
